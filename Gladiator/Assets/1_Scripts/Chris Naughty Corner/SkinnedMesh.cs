...
public class SkinnedMesh : MonoBehaviour
{
    ...
    private MeshCollider collide; // quick pointer to the mesh collider that we're updating
                                  // Function:    Start
                                  //      T$$anonymous$$s basically translates the information about the skinned mesh into
    ...
    void Start()
    {
        ...
        //else
        //{
        //    Debug.LogError(gameObject.name + ": SkinnedCollisionHelper: t$$anonymous$$s object either has no SkinnedMeshRenderer or has no MeshCollider!");
        //}
    }

    // Function:    UpdateCollisionMesh
    //  Manually recalculates the collision mesh of the skinned mesh on t$$anonymous$$s
    // object.
    public void UpdateCollisionMesh()
    {
        if (mesh != null)
        {
            ...
            // Now convert each point into local coordinates of t$$anonymous$$s object.
            for (int i = 0; i < newVert.Length; i++)
            {
